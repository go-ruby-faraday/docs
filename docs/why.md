...
`go-ruby-faraday/faraday` reimplements Ruby's `faraday` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-faraday/faraday` directly, with no Ruby runtime;
